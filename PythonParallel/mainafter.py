...
def parallelMethod(boat): # parallel
	if (fbout[boat] >= 1 and xObs[boat,0,0,0] < pastbuoy): # removes irrelevent boats from simmulation
		...
		nextStepIndex = 0
		xObs[boat,:,1,nextStepIndex] = [0,2000-boat*30]
		...
		if T[preVecObs[boat,0,0],0] <= 0 and xObs[boat,0,0,0] <= 1000 and xObs[boat,1,0,0] <= 1400 and iObsMax + 1 == KnPath:
			...
			if secondMinIndex == -1:
				nextStepIndex = minimumArgument[0]
	...
	return [xObs[boat,:,:,nextStepIndex],preVecObs[boat,:,nextStepIndex],sObs[boat,:,nextStepIndex]]

def orderBoats(Zpostloc, Tloc, finalPosition, sFinal, strat, colorVec, windBaseloc, vNew, sortPosition, evolutionStep, evolutionIndex): # one round, returning the order the boats crossed the buoy
	amountthreads = constants.amountthreads

	global windBase
	global Zpost
	global T
	windBase = windBaseloc
	Zpost = Zpostloc
	T = Tloc
