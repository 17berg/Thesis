...
amountthreads = constants.amountthreads
...
stillc = 0.1# constant that determine how good it is to stand sill
...
evolutionStep = 20 # amount of evolutionary steps
...
def nextStep(boat, iObs, xObsLoc, preVecObsLoc, xNewLoc, preVecNewLoc, minVecLoc, createIndicator, infiniteloop, tmpMinSortLoc, sIndexLoc, sObsLoc, wind,tTmp): # evaluates next viable steps for a boat
	...
	for k in range(sizeStart):
		...
		if sObsLoc[boat, iObs] >= legalMoveShiftc2:
			...
			Indextmp = ((sObsLoc[boat, iObs] < legalMoveShiftc and (degree < degree1 or distanceLoc == 0) and distanceLoc <= 1.7) #(degree < degree1 or distanceLoc == 0)
...
#################### #################### #################### #################### ####################
#################### #################### #################### #################### ####################
def parallelMethod(boat): # parallel
	...
	if iObsMax + 1 == KnPath:
		...
		if Kobsvec[riskMaxLoc] == 1 or riskMaxLoc == Kobsvec.shape[0]-1:
			...
			nextPositionindex = minVecIndextmp[nextPositionindex]
		...
		if Kobsvec[riskMaxLoc] == 1 or riskMaxLoc == Kobsvec.shape[0]-1:
			nextPositionindex = np.argsort(minVec[boat,:])[0]
			...
			nextPositionindex = minVecIndextmp[minimumArgument[0]]
	return [xObs[boat,:,:,nextPositionindex],preVecObs[boat,:,:,nextPositionindex],sObs[boat,:,nextPositionindex]]
	...
	for evolutionIndex in range(kEvolution):
		...
		########################################## Step 2
			...
			for iObsMax in range(KnPath):
				poolthreads = Pool(amountthreads) # parallel
				argsArray = np.linspace(0, amountBoats, amountBoats, endpoint=False, dtype = np.int)
				poolResult = poolthreads.map(parallelMethod, argsArray)
				poolthreads.close()
				poolthreads.join()

				for poolIndex in range(amountBoats):
					xObs[poolIndex,:,:,0] = poolResult[poolIndex][0][:,:].copy()
					preVecObs[poolIndex,:,:,0] = poolResult[poolIndex][1][:,:].copy()
					sObs[poolIndex,:,0] = poolResult[poolIndex][2][:].copy()

			xNextFinal[:,:,:] = xObs[:,:,:,0].copy()
			preVecFinal[:,:,:] = preVecObs[:,:,:,0].copy()
			sVecFinal[:,:] = sObs[:,:,0].copy()
