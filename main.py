...
def nextStep(boat, iObs, xObsLoc, preVecObsLoc, xNewLoc, preVecNewLoc, minVecLoc, createIndicator, infiniteloop, tmpMinSortLoc, sIndexLoc, sObsLoc, wind,tTmp): # evaluates next viable steps for a boat
	...
	for k in range(sizeStart):
		...
		if sObsLoc[boat, iObs] >= legalMoveShiftc2:
			...
			Indextmp = ((sObsLoc[boat, iObs] < legalMoveShiftc and degree < degree1 and distanceLoc <= 1.7) #(degree < degree1 or distanceLoc == 0)
...
def branchweight(rot, futureBranchIndex, branchIndex, minVecLoc, minVecIndextmp, minValueTmp): # the value of a path
	futureBranchIndex += 1
	if futureBranchIndex == Kobsvec.shape[0]-1-math.floor(riskc):
		...
		for addBranchIndex in range(Kobsvec[math.floor(riskc)+futureBranchIndex]):
			tmpValue = branchweight(rot, futureBranchIndex, sWidthNew[math.floor(riskc),branchIndex]+addBranchIndex+1, minVecLoc, minVecIndextmp, minValueTmp)
			if tmpValue != 0:
				minValueTmp = tmpValue
		tmpValue = branchweight(rot, futureBranchIndex, branchIndex, minVecLoc, minVecIndextmp, minValueTmp)
					...
					if iObsMax + 1 == KnPath:
						...
						if Kobsvec[riskMaxLoc] == 1 or riskMaxLoc == Kobsvec.shape[0]-1:
							...
							for amountBranchesIndex in range(amountBranches):
								branchweight(amountBranchesIndex, 0, amountBranchesIndex, minVecTmp, minVecIndextmp, 10000)
